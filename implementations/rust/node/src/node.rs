...
impl TestWorker {
    ...
    pub fn handle_send(&mut self, mut m: Message) -> Result<(), String> {
        if self.channel_address.as_string() == "00000000".to_string() { // start key exchange
    ...
    pub fn handle_receive(&mut self, m: Message) -> Result<(), String> {
        ...
        match m.message_type {
            ...
            MessageType::None => {    // MessageType::None indicates new channel
            ...
            if let Ok(c) = self.rx.try_recv() {
                ...
                match c {
                    ...
                    OckamCommand::Worker(WorkerCommand::ReceiveMessage(mut m)) => {
                        match m.message_type {
                            MessageType::Payload => {
                                println!("Worker received: {}",str::from_utf8(&m.message_body).unwrap());
                            }
                            _ => {}
                        }
...
pub fn start_node(local_socket: RouterAddress, mut onward_route: Route, payload: String, is_initiator: bool) {
    ...
    onward_route.addresses.insert(0, RouterAddress::channel_router_address_from_str("00000000").unwrap());
    ...
    if is_initiator {
        let onward_route = Route { addresses: vec![RouterAddress::worker_router_address_from_str("00010203").unwrap()] };
        let m = Message {
            onward_route,
